...
- `stations[]`: `{ station_id: string, name: string, source: 'coops'|'ndbc', type: string, latitude: number, longitude: number, distance_km?: number, state?: string, capabilities: string[], prediction_class?: string, reference_id?: string, bins?: { bin: number, depth: number | null, depth_type?: string, prediction_class?: string }[] }`
...
| `noaa_marine_get_water_level` | `date_range_exceeded` | `InvalidParams` | Requested range exceeds the selected `interval`'s CO-OPS limit — 31 days for `6min`, 365 for `hourly` and `high_low`, 3,655 for `daily_mean`. The message names the interval and its limit; a coarser interval covers a longer span in one call |
