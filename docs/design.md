...
- `enrichment.truncated` / `rows_matched` / `rows_returned` / `page_offset` / `next_offset` — the page accounting, identical in shape and rule to the other CO-OPS time-series tools: present only when the response is a page, bounded by the serialized month array against the 24,000-byte budget. 1975–2025 is 612 months, several pages
